...
pub fn perform(
    ...
    if is_multipart {
        trace!("Making multipare request because form file was given");
        if !app.form {
...
fn parse(app: &App, s: &str) -> Result<Url, ParseError> {
    if s.starts_with(":/") {
...
fn handle_session(
    ...
    if let Some(s) = session {
        trace!("Adding sesssion data to request");
        builder = s.add_to_request(builder);
        if update_session {
            trace!("Updating session with parameters from request");
